...
func TestCommandRunReturnsMinifiedJSONWithoutWriting(t *testing.T) {
	dir := t.TempDir()
	...
	if result.Command != "minify-schema" {
		t.Fatalf("expected minify-schema command, got %q", result.Command)
	}
	if result.Output != "{\"type\":\"object\"}" {
